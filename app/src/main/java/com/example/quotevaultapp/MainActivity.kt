...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Initialize PreferencesManager singleton
        val preferencesManager = com.example.quotevaultapp.data.local.PreferencesManager.getInstance(this)
        
        setContent {
            val context = LocalContext.current
            
            // Collect preferences as state
            val theme by preferencesManager.theme.collectAsState(initial = com.example.quotevaultapp.domain.model.AppTheme.SYSTEM)
            val accentColor by preferencesManager.accentColor.collectAsState(initial = com.example.quotevaultapp.domain.model.AccentColor.PURPLE)
            val fontSize by preferencesManager.fontSize.collectAsState(initial = com.example.quotevaultapp.domain.model.FontSize.MEDIUM)
            ...
            QuoteVaultTheme(
                ...
                androidx.compose.runtime.CompositionLocalProvider(
                    com.example.quotevaultapp.presentation.theme.LocalFontSize provides fontSize
                ) {
